...
const Category = ({categoryPosts, singleCategory}) => {
  const [posts, setPosts] = useState(categoryPosts)
  
  return (
<>
    <Head>
      <title key={singleCategory.name}>{singleCategory.name}</title>
      <meta key="singlecategory-metadescription" name="description" content={singleCategory.name}/>
    </Head>
    ...
    <section className='py-[6rem] flex flex-col items-center'>
          ...
          <LoadMore posts={posts} setPosts={setPosts} taxonomy={{key:"categoryName", value: singleCategory.slug}}/>
        </section>
